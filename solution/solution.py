...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        """
        # Average of the three main answer quality metrics
...
class RAGASEvaluator:
    ...
    def evaluate_relevance(self, answer: str, question: str) -> float:
        ...
        if not q_tok:
            return 1.0
        ans_tok = _tokenize(answer)
        # Proportion of question tokens found in answer
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        if not exp_tok:
            return 1.0
        ans_tok = _tokenize(answer)
        # Proportion of expected tokens found in answer
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        if not contexts:
            return 0.0
        # Compute union of all retrieved chunk tokens
        union_tok = set()
        for chunk in contexts:
            union_tok.update(_tokenize(chunk))
        score = len(exp_tok & union_tok) / len(exp_tok)
        return max(0.0, min(1.0, score))
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            overlap = len(chunk_tok & exp_tok) / len(exp_tok) if exp_tok else 0.0
            is_relevant.append(overlap >= relevance_threshold)

        num_rel = sum(is_relevant)
        if num_rel == 0:
            return 0.0
        ...
        return max(0.0, min(1.0, ap_sum / num_rel))
    ...
    def run_full_eval(
        ...
        relev = self.evaluate_relevance(answer, question)
        compl = self.evaluate_completeness(answer, expected)

        # Pass criteria: all scores >= 0.5
        passed = (faith >= 0.5) and (relev >= 0.5) and (compl >= 0.5)
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    # Sort chunks by number of overlapping words with query, descending
...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        # Build prompt using rubric guidelines
        rubric_lines = "\n".join(f"- {k}: {v}" for k, v in rubric.items())
        prompt = (
            f"Question: {question}\n"
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            f"Please output a JSON dictionary mapping each criterion to a score (0.0 to 1.0)."
        )
        response = self.judge_llm_fn(prompt)
        
        # Parse scores dictionary from response
        import json
        scores = {}
        try:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Check positional bias: first item's average vs rest of the batch average
        positional = False
        if len(scores_batch) > 1:
            ...
            if first_vals and rest_vals:
                ...
                positional = avg_first > avg_rest + 0.05
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            # Generate answer via agent function
            answer = agent_fn(pair.question)
            # Run faithfulness, relevance, completeness evals
            ...
            # Reference the original QA pair
            res.qa_pair = pair
            # Calculate retrieval precision/recall if contexts are present
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        avg_compl = sum(r.completeness for r in results) / total
    ...
    def run_regression(self, new_results: list[EvalResult], baseline_results: list[EvalResult]) -> dict:
        ...
        new_f = sum(r.faithfulness for r in new_results) / n_new if n_new > 0 else 0.0
        new_r = sum(r.relevance for r in new_results) / n_new if n_new > 0 else 0.0
        new_c = sum(r.completeness for r in new_results) / n_new if n_new > 0 else 0.0

        base_f = sum(r.faithfulness for r in baseline_results) / n_base if n_base > 0 else 0.0
        base_r = sum(r.relevance for r in baseline_results) / n_base if n_base > 0 else 0.0
        base_c = sum(r.completeness for r in baseline_results) / n_base if n_base > 0 else 0.0
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        # Map failure types to specific suggestions
        if counts.get("hallucination", 0) > 0 or counts.get("Hallucination", 0) > 0:
            ...
            suggestions.append("Enforce strict grounding in prompt (e.g., 'only use the context provided')")
        ...
        if counts.get("irrelevant", 0) > 0 or counts.get("off_topic", 0) > 0 or counts.get("Off_topic", 0) > 0:
            suggestions.append("Refine system prompt and routing mechanism to improve question relevancy")
            suggestions.append("Use query expansion or HyDE to retrieve more relevant documents")

        default_suggestions = [
            "Implement a reranker (e.g., Cohere or BGE) to prioritize relevant chunks for the generator",
            "Tune chunk overlap parameters to avoid fragmenting key information across multiple pieces",
            "Calibrate the system prompt with strict refusal instructions for out-of-scope queries"
        ]
